...
def get_roles():
    n_roles = len(st.session_state.candidate["roles"])
    st.write(f"roles = {n_roles}")
...
def get_education():
    n_education = len(st.session_state.candidate["educations"])
    st.write(f"education : {n_education}")
    education = {}
    ...
    education["date_start"] = st.date_input("start").strftime('%Y/%m/%d')
    education["date_end"] = st.date_input("end").strftime('%Y/%m/%d')
...
if field_selection == "work experiences":
    n_work_exp = len(st.session_state.candidate["work_experiences"])
    st.write(f"work experiences = {n_work_exp}")
    work_experience = {}
    ...
    work_experience["date_start"] = st.date_input("start").strftime('%Y/%m/%d')
    work_experience["date_end"] = st.date_input("end").strftime('%Y/%m/%d')
    ...
    with st.container(border=True):
        ...
        with st.container(border=True):
            ...
            if add_task:
                ...
                st.success("new task has been added")
        ...
        if add_project:
            ...
            st.success("new project has been added")
            st.session_state.tasks = []
